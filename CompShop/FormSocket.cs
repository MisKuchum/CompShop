    ...
    public partial class FormSocket : Form
    {
        ...
        private void Request()
        {
            try
            {
                ...
                {
                    case "Добавить":
                        sql = String.Format("INSERT INTO public.\"Socket\" (\"Socket_name\") VALUES ('{0}')",
                        txtName.Text);
                        break;
                    case "Изменить":
                        sql = String.Format("UPDATE public.\"Socket\" SET \"Socket_name\" = '{0}' WHERE \"Socket_ID\" = {1}", 
                            txtName.Text, txtId.Text);
                        break;
                    case "Удалить":
                        sql = String.Format("DELETE FROM public.\"Socket\" WHERE \"Socket_ID\" = {0}", txtId.Text);
                        break;
                }
