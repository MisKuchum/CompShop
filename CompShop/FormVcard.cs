...
namespace FirstApp
{
    public partial class FormVcard : Form
    {
        ...
        private void FormVcard_Load(object sender, EventArgs e)
        {
        ...
        private void Request()
        {
            try
            {
                ...
                {
                    case "Добавить":
                        sql = String.Format("INSERT INTO public.\"Video_card\" (\"Name\", \"VRAM_capacity\", \"VRAM_type\", " +
                            "\"Freq_proc\", \"Price\", \"Quantity\") VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', {5})",
                        txtName.Text, txtVidCapacity.Text, txtVidType.Text, txtProcFreq.Text, txtPrice.Text, txtQuantity.Text);
                        break;
                    case "Изменить":
                        sql = String.Format("UPDATE public.\"Video_card\" SET \"Name\" = '{0}', \"VRAM_capacity\" = '{1}', " +
                            "\"VRAM_type\" = '{2}', \"Freq_proc\" = '{3}', \"Price\" = {4}, \"Quantity\" = {5} WHERE \"VGA_ID\" = {6}", 
                            txtName.Text, txtVidCapacity.Text, txtVidType.Text, txtProcFreq.Text,
                            txtPrice.Text, txtQuantity.Text, txtId.Text);
                        break;
                    case "Удалить":
                        sql = String.Format("DELETE FROM public.\"Video_card\" WHERE \"VGA_ID\" = {0}", txtId.Text);
                        break;
                }
        ...
        private void SelectRequest()
        {
            ...
            {
                conn.Open();
                sql = "SELECT * FROM public.\"Video_card\"";
                cmd = new NpgsqlCommand(sql, conn);
                dt = new DataTable();
                dt.Load(cmd.ExecuteReader());
                conn.Close();
        ...
        private void TxtActivation()
        {
            switch (cbAction.Text)
            {
                case "Добавить":
                    ...
                    txtVidCapacity.Enabled = true;
                    txtVidType.Enabled = true;
                    txtProcFreq.Enabled = true;
                ...
                case "Удалить":
                    ...
                    txtVidCapacity.Enabled = false;
                    txtVidType.Enabled = false;
                    txtProcFreq.Enabled = false;
